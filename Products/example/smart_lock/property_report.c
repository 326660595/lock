...
// {
//     uint64_t * nowtime = get_timeStamp_tiemr_cb();
//     printf("nowtime is = %d\n",(uint32_t)(*nowtime));

//     uint64_t timestamp = (aos_now_ms() - basic_time)/1000;

//     printf("timestamp=%d;;%x",(uint32_t)timestamp,(uint32_t)timestamp);
//     LOG_TRACE("ntp_time_ms=%d,,basic_time=%d,ntp_time_s=%d,time=%d\n",ntp_time_ms,basic_time,ntp_time_ms/1000,(uint32_t)*nowtime);
// }
uint64_t local_update_timestamp(void)
{
    uint64_t time = (aos_now_ms() - basic_time)/1000;
    //printf("time is ++++=%d",(uint32_t)time);
    return time;
}
void ntp_timer_update(const char *str)
{
    ...
    snprintf(ntime0,11,"%s",str);//snprintf(buf,i-1,"%s",p-i+2);
    printf("ntp time:start_ntime0: %s\n",ntime0);
    *nowtime = atol(ntime0);
    //printf(" *nowtime = %d\n",(uint32_t)*nowtime);
