...
#define COMBO_COMMON_SERV_POST_FMT     "{\"id\":\"%d\",\"method\":\"%s\",\"params\":%s,\"version\":\"%s\"}"
...
static void combo_set_dev_status_cb(uint8_t *buffer, uint32_t length)
{
    if ((g_combo_rx.p_rx_buf != NULL) || (g_combo_rx.rx_len != 0)) {
        LOG("combo set dev rx queue full!!");
    } else {
        if (g_combo_bind.need_sign && !g_combo_bind.sign_state) {
            LOG("BLE_SET discarded because not signed");
        } else {
...
static void combo_control_handler(uint8_t *buffer, uint32_t length)
{
    ...
    if ((root != NULL) && cJSON_IsObject(root)) {
        ...
        if ((msg_id != NULL) && (method != NULL) && (params != NULL) && cJSON_IsObject(params)) {
            ...
            LOG("combo json params Error(%s)(%d)", (char *)buffer, length);
            json_valid = 0;
        }
    } else {
        LOG("combo json parse Error");
        json_valid = 0;
    }
    ...
    if (json_valid) {
        ...
        if ((strncmp(method->valuestring, COMBO_TSL_METHOD_PROPER_SET, strlen(COMBO_TSL_METHOD_PROPER_SET)) == 0)
        ...
        } else if ((strncmp(method->valuestring, COMBO_TSL_METHOD_PROPER_GET, strlen(COMBO_TSL_METHOD_PROPER_GET)) == 0)
                && g_combo_property_get_cb) {
            char *response = NULL;
            g_combo_property_get_cb(COMBO_DEFAULT_DEVID, p_param, strlen(p_param), &response, &response_len);
            if (response != NULL && response_len > 0) {
                // property get response exist
                combo_tsl_ack(COMBO_TSL_SUCCESS, response, response_len);
                HAL_Free(response);
            } else {
                combo_tsl_ack(COMBO_TSL_FIND_PROPERTY_FAIL, COMBO_TSL_EMPTY_ACK, strlen(COMBO_TSL_EMPTY_ACK));
            }
        } else if ((strncmp(method->valuestring, COMBO_COMMON_SERV_PREFIX, strlen(COMBO_COMMON_SERV_PREFIX)) == 0)
            ...
            LOG("combo no callback found");
            combo_tsl_ack(COMBO_TSL_FIND_SERVICE_FAIL, COMBO_TSL_EMPTY_ACK, strlen(COMBO_TSL_EMPTY_ACK));
        }
        if (p_param) {
            HAL_Free(p_param);
        }
...
int combo_tsl_ack(uint16_t err_code, uint8_t *buffer, uint32_t length)
{
    ...
    if (!rsp_src) {
        LOG("combo_tsl_ack: alloc fail!");
        return -1;
    }

    HAL_Snprintf(rsp_src,
                rsp_len,
                COMBO_TSL_ACK_FMT,
                g_combo_rx.rx_msg_id,
                err_code,
                (char *)buffer);
    LOG("combo_tsl_ack:%.*s", strlen(rsp_src), rsp_src);
    combo_send_data_use_ind(0x03, rsp_src, strlen(rsp_src));
    LOG("combo_tsl_ack: %d > %d", rsp_len, strlen(rsp_src));
    HAL_Free(rsp_src);
}
...
int combo_status_report(uint8_t *buffer, uint32_t length)
{
    ...
    if (!rpt_src) {
        LOG("combo_status_report: alloc fail!");
        return -1;
    }

    HAL_Snprintf(rpt_src,
                rpt_len,
                COMBO_COMMON_SERV_POST_FMT,
                g_combo_tx.tx_msg_id++,
                COMBO_TSL_METHOD_PROPER_POST,
                (char *)buffer,
                COMBO_TSL_PROTOCOL_VERSION);
    LOG("combo_status_report:%.*s", strlen(rpt_src), rpt_src);
    combo_send_data_use_ind(0x01, rpt_src, strlen(rpt_src));
    LOG("combo_status_report: %d > %d", rpt_len, strlen(rpt_src));
    HAL_Free(rpt_src);
}
