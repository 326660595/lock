...
int get_wifi_upgrade_file_header(FIRMWARE_INFO *wifi_file_header)
{
    ...
    int offset=0;
    int ret=hal_flash_read(HAL_PARTITION_WIFI_OTA_HEADER,&offset,(unsigned char*)wifi_file_header,sizeof(FIRMWARE_INFO));
    if(ret==0)
    {
        //����wifi_file_header->version������WIFI file size
